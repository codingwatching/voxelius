...
namespace detail
{
static inline const bool checkOpt(const std::string &argv)
{
...
void cmdline::append(int argc, char **argv)
{
    for(int i = 1; i < argc; i++) {
        if(detail::checkOpt(argv[i])) {
            std::string opt = detail::getOpt(argv[i]);
            if((i + 1 < argc) && !detail::checkOpt(argv[i + 1])) {
